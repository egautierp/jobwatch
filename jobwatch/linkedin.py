...
SKIP_LINE = re.compile(
    r"^(view job|apply|easy apply|actively recruiting|promoted|be an early applicant|"
    r"\d+ (school alumni|connections?|applicants?)|see all jobs|your job alert)", re.I)
...
def parse_alert(body):
    ...
    for raw in body.splitlines():
        ...
        if m:
            ...
            if info:
                # Title, company and location sit directly above the link.
                tail = info[-3:] if len(info) >= 3 else info + [""] * (3 - len(info))
...
def fetch(user, password, days=3):
